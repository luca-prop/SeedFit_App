...
- 기준 URL: 기존 임시 Vercel URL이 아니라 MVP-025 PR에서 새로 생성되는 Preview URL을 기준으로 검증한다.
...
- 새 PR Preview URL에서 `/app/preview-health`가 `READY` 상태다.
- 새 PR Preview URL에서 `/app/results?budgetMin=1000000000&budgetMax=1500000000`가 `Reverse Filter 처리 중 오류가 발생했습니다.`를 노출하지 않는다.
