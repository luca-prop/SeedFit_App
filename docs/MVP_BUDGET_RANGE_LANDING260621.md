...
- 결과 카드와 1:1 비교 프리뷰 사이 여백을 줄여 같은 화면 안에서 선택 변화가 바로 보이도록 했습니다.
- B2B Verified 매물 기능은 MVP 30개 TASK 범위에서 제외되었으므로, 랜딩 카피는 `검증된 구역 우선 공개`, `큐레이션 데이터 기반`, `Curated DB 기반 비교`처럼 데이터 큐레이션 신뢰 메시지로 정리했습니다.
- 외부 랜딩 카피에는 `Golden Sample`, `Naver LAND` 같은 내부 작업명/원천명을 직접 쓰지 않고, `운영팀 검수`, `공개 자료`, `단지 기준가`처럼 사용자가 이해할 수 있는 신뢰 표현을 사용합니다.
...
1:1 비교 프리뷰는 다음 구조로 노출합니다.

- 1행: 좌측 `재개발구역`, 우측 `기축 레퍼런스`
- 2행: `월계동신`, `다산e편한세상자이`, `주공뜨란채`의 필요 실투자금(LTV)과 최근시세

스캐터 프리뷰에서 동일 단계·동일 예산 구역이 겹치면 `frontend/app/lib/scatterDates.ts`의 진행일을 기준으로 더 빠른 구역을 오른쪽에 배치합니다. 진행일도 같으면 구역명 기준의 안정적 타이브레이커로 X축을 미세 조정해 겹침을 피합니다.
