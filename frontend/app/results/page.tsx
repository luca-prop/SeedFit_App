...
/**
 * 역산 결과 리스트 페이지 — Mobile-First
 * 
 * @mobile_first
 * - 모바일: 단일 열 카드 리스트 (풀 너비, 터치 타겟 확대)
 * - md+: 2열 그리드 카드 레이아웃 유지
 */
  ...
  return (
    <div className="container mx-auto px-4 py-6 md:py-8 max-w-5xl">
      ...
      <div className="mb-5 md:mb-6">
        <Button variant="ghost" size="sm" onClick={() => router.push("/")} className="mb-3 md:mb-4 -ml-2 min-h-[44px]">
      ...
      {/* Freshness Alert */}
      <Alert className="mb-5 md:mb-6 bg-orange-50 border-orange-200 text-orange-800">
        <AlertTriangle className="h-4 w-4" />
        <AlertTitle className="text-sm md:text-base">데이터 갱신 지연 안내</AlertTitle>
        <AlertDescription className="text-xs md:text-sm">마지막 데이터 갱신일: 2026.03.01 (45일 전). 실제 시세와 차이가 있을 수 있습니다.</AlertDescription>
      </Alert>
      ...
      {filtered.length > 0 ? (
        ...
        <Card className="flex flex-col items-center justify-center p-8 md:p-12 text-center border-dashed bg-white">
          ...
          <p className="text-sm md:text-base text-gray-500 mb-6">예산을 높이거나 알림을 설정해 새 매물 출현 시 알림을 받아보세요.</p>
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            <Button className="min-h-[44px]">
              <Bell className="h-4 w-4 mr-2" />
              매물 출현 알림 받기
            </Button>
            <Button variant="outline" className="min-h-[44px]" onClick={() => router.push("/")}>예산 수정하기</Button>
          </div>
        </Card>
      )}
    </div>
