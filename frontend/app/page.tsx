 ...
 * - '시작하기'를 누르면 `/reverse-filter?cash={budget}` 라우트로 이동하여 역방향 필터링을 수행합니다.
 */
export default function HomePage() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-10rem)] px-4 py-16">
      {/* Hero */}
      <div className="text-center mb-12 max-w-2xl">
        <span className="inline-block bg-blue-50 text-blue-700 text-sm font-semibold px-3 py-1 rounded-full mb-4">
          예산 맞춤 투자 분석 플랫폼
        </span>
      ...
      <div className="w-full max-w-xl space-y-4">
        <div className="flex gap-3">
          ...
          <Button size="lg" className="h-14 px-6 rounded-xl text-base" onClick={handleSearch}>
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {QUICK_BUDGETS.map(({ label, value }) => (
            <button
              ...
              className="text-sm px-3 py-1.5 rounded-lg border border-gray-200 bg-white hover:bg-gray-50 hover:border-primary/30 transition-colors text-gray-700"
            >
              {label}
            </button>
          ))}
